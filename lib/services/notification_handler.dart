...
class NotificationHandler {
  static receiveNotification(BuildContext context, GlobalKey<ScaffoldState> scaffoldKey) {
  ...
  static navigateToScreen(BuildContext context, String type, String objectId) async {
    switch (type) {
      case 'message':
        Navigator.of(context).pushNamed('/conversation', arguments: {'other_uid': objectId});
        break;
  ...
  static sendNotification(String receiverId, String title, String body, String objectId, String type) async {
  ...
  clearNotificationsNumber() async {
    await usersRef.document(Constants.currentUserID).updateData({'notificationsNumber': 0});
  }
}
