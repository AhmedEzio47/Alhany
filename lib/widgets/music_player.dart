...
class _MusicPlayerState extends State<MusicPlayer> {
  ...
  void initAudioPlayer() async {
    ...
    for (Melody melody in widget.melodyList) {
      if (melody.audioUrl != null) urlList.add(melody.audioUrl!);
    }
  ...
  Widget favouriteBtn() {
    return Padding(
      ...
      child: InkWell(
        onTap: () async {
          if (widget.melodyList[index].id != null)
            _isFavourite
  ...
  Widget downloadOrOptions() {
    return Constants.isAdmin
        ...
        : widget.melodyList[index].authorId != null
            ? Padding(
                padding: const EdgeInsets.only(left: 8.0),
                child: InkWell(
                  onTap: () async {
  ...
  editImage() async {
    ...
    if (image != null) {
      ...
      String url = await AppUtil().uploadFile(image, context,
          '/melodies_images/${widget.melodyList[index].id}$ext');
      await melodiesRef
          .doc(widget.melodyList[index].id)
  ...
  editName() async {
    if (widget.melodyList[index].name != null)
      setState(() {
  ...
  deleteMelody() async {
    AppUtil.showAlertDialog(
        ...
        firstFunc: () async {
          ...
          if (widget.melodyList[index].singer != null) {
  ...
  void _downloadMelody() async {
    ...
    if (token.tokenId != null) {
      ...
      if (widget.melodyList[index].audioUrl != null) {
        ...
        if (widget.melodyList[index].levelUrls != null)
